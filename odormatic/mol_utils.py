...
def clean_matrices(Dream, Dravnieks, remove_inds_DRV, remove_mols_DRM,
                   remove_mols_DRV):
...
def load_FT_SCALEALL(filename, words, dimension=None):
    # Load word vectors for specified words from specified file in FastText format.
    assert os.path.isfile(filename)
    f = open(filename)
    all_words = []
    ...
    for line in f.readlines():
        line = line.split()
        embeds.append([float(i) for i in line[1:]])
        all_words.append(line[0])

    # Skip the first line
    embeds = np.array(embeds[1:])
    # projection = TruncatedSVD(n_components=dimension).fit_transform(embeds)
    if dimension:
        projection = TruncatedSVD(n_components=dimension).fit_transform(embeds)
    else:
        projection = StandardScaler().fit_transform(embeds)

    model = {}
    for i, word in enumerate(all_words):
        if word in words:
            model[word] = projection[i]

    model.pop('---', None)  #delete '---' from the model
    return model


def load_FT_SCALELOCAL(filename, words, dimension=None):
    # Load word vectors for specified words from specified file in FastText format.
    f = open(filename)
    all_words = []
    embeds = []
...
def corrcoef(arr1, arr2):
    ...
    # Correlation coefficient matrix will be 2x2
    assert result.shape == (2, 2)
    # The diagonal is not of interest to us
    assert np.isclose(result[0, 0], result[1, 1])
    # Because it is 1
    assert np.isclose(result[0, 0], 1)
    # We are interested in the correlation between the two matrices
    return result[1, 0]
...
def preprocess(DRM_words,
               DRV_words,
               do_contextual,
               expandSet=True):
    if do_contextual:
        # In the contextual problem, we will not remove or modify any descrptors.
        # Just convert to lower.
